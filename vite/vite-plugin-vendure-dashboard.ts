 ...
 */
export function vendureDashboardPlugin(
  options: VitePluginVendureDashboardOptions
): PluginOption[] {
  const tempDir =
    options.tempCompilationDir ??
    path.join(import.meta.dirname, "./.vendure-dashboard-temp")
  const normalizedVendureConfigPath = getNormalizedVendureConfigPath(
    options.vendureConfigPath
  )
...
function getDashboardPackageRoot(): string {
  // Since we're in the dashboard package itself, use the current working directory
  return fixWindowsPath(process.cwd())
}

/**
 * Get the normalized path to the Vendure config file given either a string or URL.
 */
export function getNormalizedVendureConfigPath(
  vendureConfigPath: string | URL
): string {
  const stringPath =
    typeof vendureConfigPath === "string"
      ? vendureConfigPath
      : vendureConfigPath.href
