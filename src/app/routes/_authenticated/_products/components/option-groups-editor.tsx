...
function createOptionValueSchema(t: (key: string) => string) {
    return z.object({
...
function createOptionGroupSchema(t: (key: string) => string) {
    return z.object({
        name: z.string().min(1, { message: t('Option name is required') }),
        values: z.array(createOptionValueSchema(t)).min(1, { message: t('At least one value is required') }),
    });
}
