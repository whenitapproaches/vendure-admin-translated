...
export function FulfillmentDetails({ order, fulfillment, onSuccess }: Readonly<FulfillmentDetailsProps>) {
    ...
    const getFulfillmentActions = () => {
        ...
        if (suggested) {
            actions.push({
                label: `${t('Transition to')} ${suggested}`,
    ...
    return (
        <div className="space-y-1 p-3 border rounded-md">
            <div className="space-y-1">
                <LabeledData label={t('Fulfillment ID')} value={fulfillment.id.slice(-8)} />
                <LabeledData label={t('Method')} value={fulfillment.method} />
                <LabeledData label={t('State')} value={fulfillment.state} />
                {fulfillment.trackingCode && (
                    <LabeledData label={t('Tracking code')} value={fulfillment.trackingCode} />
                )}
                <LabeledData label={t('Created')} value={formatDate(fulfillment.createdAt)} />
            </div>
