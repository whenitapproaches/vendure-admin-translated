...
function createCustomizeColumns(currencyCode: string, t: (key: string) => string) {
    return {
        featuredAsset: {
            header: () => t('Image'),
...
export function OrderTable({ order, pageId }: Readonly<OrderTableProps>) {
    ...
    const customizeColumns = useMemo(() => createCustomizeColumns(currencyCode, t), [currencyCode, t]);
