...
export function MetricsWidget() {
    ...
    const chartData = useMemo(() => {
        const entry = (data as any)?.metricSummary?.at(0);
        if (!entry) {
            return undefined;
        }
            ...
            actions={
                <div className="flex gap-1">
                    <Tabs defaultValue={dataType} onValueChange={value => setDataType(value as DATA_TYPES)}>
                        <TabsList>
                            <TabsTrigger value={DATA_TYPES.OrderCount}>{t("Order Count")}</TabsTrigger>
                            <TabsTrigger value={DATA_TYPES.OrderTotal}>{t("Order Total")}</TabsTrigger>
            ...
            {chartData && (
                <MetricsChart
                    formatValue={value => {
                        ...
                        return formatCurrency(value * 100, 'VND', 0);
